...
import { AgentHandoff, AgentPersona, KnowledgeBase as KB } from '../types';
...
export default function KnowledgeBase() {
  ...
  const togglePin = async (id: string, currentPinned: boolean) => {
    await supabase.from('knowledge_base').update({ is_pinned: !currentPinned }).eq('id', id);

    setKnowledge(knowledge.map(k => (k.id === id ? { ...k, is_pinned: !currentPinned } : k)));
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-500/20 rounded-xl p-6 hover:border-green-500/40 transition-all">
          ...
          <div className="text-2xl font-bold mb-1">
            {knowledge.filter(k => k.category === 'snippet').length}
          </div>
        ...
        <div className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 border border-yellow-500/20 rounded-xl p-6 hover:border-yellow-500/40 transition-all">
          ...
          <div className="text-2xl font-bold mb-1">
            {knowledge.filter(k => k.category === 'tip').length}
          </div>
      ...
      <div>
        <h2 className="text-xl font-bold mb-4">All Knowledge ({filteredKnowledge.length})</h2>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    >
                      <Pin
                        size={14}
                        className={
                          entry.is_pinned ? 'text-cyan-400 fill-cyan-400' : 'text-slate-400'
                        }
                      />
                    </button>
                    <button
